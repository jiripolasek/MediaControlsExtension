...
internal static class WorkerApplicationTests
{
    ...
    public static IReadOnlyDictionary<string, WorkerBackendFactory> Factories { get; } =
        new Dictionary<string, WorkerBackendFactory>(StringComparer.Ordinal)
        {
            ...
            ["application-hang-command"] = static (context, _) => new SyntheticBackend("hang-command", context),
            ["application-hang-snapshot"] = static (context, _) => new SyntheticBackend("hang-snapshot", context)
        };
    ...
    [
        ...
        ("Worker application dispatches a plain backend without activation",
            () => DispatchAsync("application-plain", directory)),
    ...
    private static async Task InvalidLaunchAsync()
    {
        ...
        {
            var code = await WorkerApplication.RunAsync(args, Factories).WaitAsync(TimeSpan.FromSeconds(2))
    ...
    private static async Task InvalidHandshakeAsync()
    {
        ...
        var run = WorkerApplication.RunAsync([
            "--worker", name, owner.ToString("D"),
            Environment.ProcessId.ToString(CultureInfo.InvariantCulture), "known"
        ], factories);
        await pipe.WaitForConnectionAsync().WaitAsync(TimeSpan.FromSeconds(3)).ConfigureAwait(false);
        await using var endpoint = await WorkerRpcEndpoint.CreateAsync(pipe, true, default).ConfigureAwait(false);
        var rpc = endpoint.Rpc.Attach<IWorkerRpc>();
        endpoint.Rpc.StartListening();
        try
        {
            await rpc.InitializeAsync(new(PipeProtocol.Version, owner, "different", new(0, []), false, null,
                    TimeSpan.FromSeconds(5), TimeSpan.FromSeconds(30), TimeSpan.FromSeconds(45)), default)
                .ConfigureAwait(false);
        }
    ...
    private static async Task DispatchAsync(string id, string directory)
    {
        ...
        {
            Logging = new WorkerLoggingOptions(logDirectory, false),
            ActivateSource = (request, _) =>
            {
                HostingTests.Check(request.ApplicationId == "spike.player",
                    "The factory lost the source activation context.");
        ...
        HostingTests.Check(
            (await backend.ExecuteAsync(HostingTests.Command(snapshot, MediaOperation.Play), CancellationToken.None)
        ...
        if (canActivate)
        {
            HostingTests.Check(
                (await backend
                    .ExecuteAsync(HostingTests.Command(snapshot, MediaOperation.ActivateSource), CancellationToken.None)
                    .ConfigureAwait(false)).Status == MediaBackendCommandStatus.Completed && activations == 1,
        ...
        HostingTests.Check(worker.HasExited && worker.ExitCode == 0,
            "The worker application did not exit gracefully with its owner scope.");
        HostingTests.Check(
            Directory.GetFiles(logDirectory, "log-worker-*.txt").Any(file =>
                File.ReadAllText(file).Contains($"Selected {id} factory.", StringComparison.Ordinal)),
            "The selected factory did not receive the configured logger factory.");
        HostingTests.Check(
            Directory.GetFiles(logDirectory, "log-worker-*.txt").All(file =>
                !File.ReadAllText(file).Contains("exceeded ", StringComparison.Ordinal)),
            "Normal shutdown reported a watchdog timeout.");
    }

    private static async Task WatchdogLoggingAsync(bool snapshotTimeout, string directory)
    {
        var logDirectory = Path.Combine(directory, $"watchdog-{Guid.NewGuid():N}");
        await using var backend = new OutOfProcessMediaBackend(
            Program.Options(snapshotTimeout ? "application-hang-snapshot" : "application-hang-command") with
            {
                MaximumRestarts = 0,
                Logging = new WorkerLoggingOptions(logDirectory, false),
        ...
        else
        {
            ...
            var command = backend.ExecuteAsync(HostingTests.Command(snapshot, MediaOperation.SkipNext), cancel.Token);
        ...
        var expected = snapshotTimeout
            ? "The worker snapshot read exceeded ObservationTimeout."
            : "The worker command exceeded RequestTimeout.";
        await HostingTests.EventuallyAsync(() => Directory.GetFiles(logDirectory, "log-worker-*.txt")
            .Any(file => ReadLog(file).Contains(expected, StringComparison.Ordinal))).ConfigureAwait(false);

...
{
    public static async Task<int> RunAsync(string resultsDirectory)
    {
        ...
        var tests = new (string Name, Func<Task> Run)[]
        {
            ...
            ("Oversized frame terminates the worker", () => InvalidMessageAsync(oversized: true)),
            ("Protocol mismatch terminates the worker", () => InvalidMessageAsync(oversized: false)),
            ...
            ("Activation timeout fences a late owner result", () => LostActivationAsync(disconnect: false)),
            ("Worker disconnect fences a late owner activation result", () => LostActivationAsync(disconnect: true)),
        ...
        tests = [.. tests, .. ProtocolTests.Cases, .. MemoryMaintenanceTests.Cases, .. WorkerApplicationTests.Cases(resultsDirectory), .. DummyBackendTests.Cases];
        tests = [.. tests, .. ReviewRegressionTests.Cases(resultsDirectory), .. ReviewTimeoutTests.Cases, .. ReviewRecoveryTests.Cases];
        ...
        results.Add($"{tests.Length - failures}/{tests.Length} passed. Runtime: {System.Runtime.InteropServices.RuntimeInformation.FrameworkDescription}; architecture: {System.Runtime.InteropServices.RuntimeInformation.ProcessArchitecture}; dynamic code: {System.Runtime.CompilerServices.RuntimeFeature.IsDynamicCodeSupported}.");
    ...
    internal static async Task<MediaBackendSnapshot> WaitForSnapshotAsync(IMediaBackend backend, TimeSpan? timeout = null)
    {
        MediaBackendSnapshot? result = null;
        await EventuallyAsync(async () =>
        {
            result = await backend.ReadSnapshotAsync(CancellationToken.None).ConfigureAwait(false);
            return result.Connection.Status == MediaConnectionStatus.Connected && result.Availability == MediaControlAvailability.Available;
    ...
    private static async Task ContractRoundTripAsync()
    {
        ...
        await backend.ApplySourcePolicyAsync(new(1, ["another.player"]), CancellationToken.None).ConfigureAwait(false);
        ...
        Check(snapshot.SourcePolicyRevision == 1 && snapshot.Sessions.Length == 1, "Initial policy/snapshot did not arrive.");
        Check(backend.WorkerPackageFullName == OwnedWorkerProcess.CurrentPackageFullName, "The worker has the wrong package identity.");
        ...
        Check(artwork is not null && artwork.Data.Span.SequenceEqual(new byte[] { 1, 2, 3, 1 }), "Artwork bytes changed in transit.");
        Check((await backend.ExecuteAsync(Command(snapshot, MediaOperation.Play), CancellationToken.None).ConfigureAwait(false)).Status == MediaBackendCommandStatus.Completed, "Play failed.");
        await EventuallyAsync(async () => (await backend.ReadSnapshotAsync(CancellationToken.None).ConfigureAwait(false)).Sessions[0].PlaybackState == MediaPlaybackState.Playing).ConfigureAwait(false);
        backend.InvalidateObservations([new(snapshot.Sessions[0].Id, MediaBackendObservationChanges.Playback)]);
        await EventuallyAsync(async () => (await backend.ReadSnapshotAsync(CancellationToken.None).ConfigureAwait(false)).Sessions[0].MediaProperties.Subtitle.Contains("Invalidations: 1", StringComparison.Ordinal)).ConfigureAwait(false);
        await backend.ExecuteAsync(Command(snapshot, MediaOperation.SkipNext), CancellationToken.None).ConfigureAwait(false);
        await EventuallyAsync(async () => (await backend.ReadSnapshotAsync(CancellationToken.None).ConfigureAwait(false)).Sessions[0].MediaProperties.Title == "Next").ConfigureAwait(false);
        Check(await backend.GetArtworkAsync(key, CancellationToken.None).ConfigureAwait(false) is null, "Obsolete artwork key was accepted.");
        await backend.ExecuteAsync(Command(snapshot, MediaOperation.SkipPrevious), CancellationToken.None).ConfigureAwait(false);
        await EventuallyAsync(async () => (await backend.ReadSnapshotAsync(CancellationToken.None).ConfigureAwait(false)).Sessions[0].BindingGeneration > snapshot.Sessions[0].BindingGeneration).ConfigureAwait(false);
        Check((await backend.ExecuteAsync(Command(snapshot, MediaOperation.Play), CancellationToken.None).ConfigureAwait(false)).Status == MediaBackendCommandStatus.SessionGone, "Old binding was accepted.");
        await backend.ApplySourcePolicyAsync(new(2, ["spike.player"]), CancellationToken.None).ConfigureAwait(false);
        ...
        await backend.ApplySourcePolicyAsync(new(3, []), CancellationToken.None).ConfigureAwait(false);
        await EventuallyAsync(async () => (await backend.ReadSnapshotAsync(CancellationToken.None).ConfigureAwait(false)).Sessions.Length == 1).ConfigureAwait(false);
        var restored = await WaitForSnapshotAsync(backend).ConfigureAwait(false);
        Check(restored.SourcePolicyRevision == 3 && restored.Sessions.Length == 1, "Updated policy did not restore discovery.");
    }

    private static async Task PlainBackendAsync()
    {
        await using var backend = new OutOfProcessMediaBackend(Program.Options("plain"));
        await backend.ApplySourcePolicyAsync(new(4, []), CancellationToken.None).ConfigureAwait(false);
        await backend.StartAsync(CancellationToken.None).ConfigureAwait(false);
        var snapshot = await WaitForSnapshotAsync(backend).ConfigureAwait(false);
        Check(snapshot.SourcePolicyRevision == 4 && snapshot.Sessions.Length == 1, "A plain backend could not acknowledge an empty policy.");
    }
    ...
    private static async Task IndependentOwnersAsync()
    {
        ...
        await Task.WhenAll(a.StartAsync(CancellationToken.None), b.StartAsync(CancellationToken.None)).ConfigureAwait(false);
        ...
        Check((await b.ExecuteAsync(Command(snapshot, MediaOperation.Play), CancellationToken.None).ConfigureAwait(false)).Status == MediaBackendCommandStatus.Completed,
    ...
    private static async Task LargeArtworkAsync()
    {
        ...
        await backend.StartAsync(CancellationToken.None).ConfigureAwait(false);
        var snapshot = await WaitForSnapshotAsync(backend).ConfigureAwait(false);
        var read = backend.GetArtworkAsync(snapshot.Sessions[0].MediaProperties.Artwork!.Value, CancellationToken.None).AsTask();
        var control = await backend.ExecuteAsync(Command(snapshot, MediaOperation.Play), CancellationToken.None).ConfigureAwait(false);
    ...
    private static async Task OversizedArtworkAsync()
    {
        ...
        await backend.StartAsync(CancellationToken.None).ConfigureAwait(false);
        var snapshot = await WaitForSnapshotAsync(backend).ConfigureAwait(false);
        Check(await backend.GetArtworkAsync(snapshot.Sessions[0].MediaProperties.Artwork!.Value, CancellationToken.None).ConfigureAwait(false) is null,
            "Unsupported artwork was accepted.");
        Check((await backend.ExecuteAsync(Command(snapshot, MediaOperation.Play), CancellationToken.None).ConfigureAwait(false)).Status == MediaBackendCommandStatus.Completed,
    ...
    private static async Task ActivationAsync()
    {
        ...
        await using var backend = new OutOfProcessMediaBackend(Program.Options("activation") with
        {
            ActivateSource = (request, token) =>
            {
                ...
                Check(Environment.ProcessId == ownerPid && request.ApplicationId == "spike.player" && request.CommandId > 0,
        ...
        await backend.StartAsync(CancellationToken.None).ConfigureAwait(false);
        var snapshot = await WaitForSnapshotAsync(backend).ConfigureAwait(false);
        Check(snapshot.Sessions[0].Capabilities.HasFlag(MediaCapabilities.ActivateSource), "Activation capability was not negotiated.");
        Check((await backend.ExecuteAsync(Command(snapshot, MediaOperation.ActivateSource), CancellationToken.None).ConfigureAwait(false)).Status == MediaBackendCommandStatus.Completed,
    ...
    private static async Task InvalidActivationAsync()
    {
        ...
        {
            ActivateSource = (_, _) => { calls++; return Task.FromResult(true); },
        });
        await backend.StartAsync(CancellationToken.None).ConfigureAwait(false);
        var snapshot = await WaitForSnapshotAsync(backend).ConfigureAwait(false);
        Check((await backend.ExecuteAsync(Command(snapshot, MediaOperation.ActivateSource), CancellationToken.None).ConfigureAwait(false)).Status == MediaBackendCommandStatus.Failed,
    ...
    private static async Task StaleActivationAsync(bool exclude)
    {
        ...
        await EventuallyAsync(async () => (await backend.ReadSnapshotAsync(CancellationToken.None).ConfigureAwait(false)).Sessions[0].MediaProperties.Title == "Activating").ConfigureAwait(false);
        if (exclude)
        {
            await backend.ApplySourcePolicyAsync(new(1, ["spike.player"]), CancellationToken.None).ConfigureAwait(false);
        }
        else
        {
            await backend.ExecuteAsync(Command(snapshot, MediaOperation.SkipPrevious), CancellationToken.None).ConfigureAwait(false);
    ...
    private static async Task LostActivationAsync(bool disconnect)
    {
        ...
        await using var backend = new OutOfProcessMediaBackend(Program.Options("activation") with
        {
            ...
            ActivateSource = (_, _) => { Interlocked.Increment(ref calls); entered.TrySetResult(); return release.Task; },
        });
        try
        {
            await backend.StartAsync(CancellationToken.None).ConfigureAwait(false);
            ...
            Check((await backend.ExecuteAsync(Command(snapshot, MediaOperation.Play), CancellationToken.None).ConfigureAwait(false)).Status == MediaBackendCommandStatus.SessionGone,
                "A command from the old activation lifetime reached its replacement.");
            Check((await backend.ExecuteAsync(Command(replacement, MediaOperation.Play), CancellationToken.None).ConfigureAwait(false)).Status == MediaBackendCommandStatus.Completed && calls == 1,
    ...
    private static async Task RestartBudgetAsync()
    {
        ...
        await backend.StartAsync(CancellationToken.None).ConfigureAwait(false);
        await EventuallyAsync(async () => (await backend.ReadSnapshotAsync(CancellationToken.None).ConfigureAwait(false)).Connection.DiagnosticMessage?.Contains("restart budget exhausted", StringComparison.Ordinal) == true).ConfigureAwait(false);
        Check(backend.RestartCount == 2 && backend.WorkerProcessId == 0, "The worker continued restarting after its budget was exhausted.");
    }

    private static async Task PolicyDuringDisconnectAsync()
    {
        await using var backend = new OutOfProcessMediaBackend(Program.Options("synthetic") with { RestartDelay = TimeSpan.FromMilliseconds(500) });
        await backend.StartAsync(CancellationToken.None).ConfigureAwait(false);
        await WaitForSnapshotAsync(backend).ConfigureAwait(false);
        var epoch = backend.WorkerEpoch;
        backend.DisconnectWorker();
        await backend.ApplySourcePolicyAsync(new(9, ["spike.player"]), CancellationToken.None).ConfigureAwait(false);
        await EventuallyAsync(() => backend.WorkerEpoch != epoch).ConfigureAwait(false);
        var snapshot = await WaitForSnapshotAsync(backend).ConfigureAwait(false);
        Check(snapshot.SourcePolicyRevision == 9 && snapshot.Sessions.IsEmpty, "The replacement ignored a policy accepted during disconnect.");
    }
    ...
    private static async Task LateArtworkAsync()
    {
        ...
        await backend.StartAsync(CancellationToken.None).ConfigureAwait(false);
        var snapshot = await WaitForSnapshotAsync(backend).ConfigureAwait(false);
        var request = backend.GetArtworkAsync(snapshot.Sessions[0].MediaProperties.Artwork!.Value, CancellationToken.None).AsTask();
        await EventuallyAsync(async () => (await backend.ReadSnapshotAsync(CancellationToken.None).ConfigureAwait(false)).Sessions[0].MediaProperties.Subtitle.Contains("Artwork reads: 1", StringComparison.Ordinal)).ConfigureAwait(false);
        await backend.ExecuteAsync(Command(snapshot, MediaOperation.SkipNext), CancellationToken.None).ConfigureAwait(false);
        await EventuallyAsync(async () => (await backend.ReadSnapshotAsync(CancellationToken.None).ConfigureAwait(false)).Sessions[0].MediaProperties.Title == "Next").ConfigureAwait(false);
    ...
    private static async Task RecoveryAsync()
    {
        ...
        try
        {
            await backend.ApplySourcePolicyAsync(new(7, ["excluded.player"]), CancellationToken.None).ConfigureAwait(false);
            await backend.StartAsync(CancellationToken.None).ConfigureAwait(false);
            ...
            await EventuallyAsync(async () => (await backend.ReadSnapshotAsync(CancellationToken.None).ConfigureAwait(false)).Sessions[0].MediaProperties.Title == "Next").ConfigureAwait(false);
            worker.Kill();
            Check((await pending.ConfigureAwait(false)).Status == MediaBackendCommandStatus.Unavailable, "Lost reply was reported as success.");
            await EventuallyAsync(() => backend.WorkerEpoch != epoch && backend.WorkerProcessId != 0).ConfigureAwait(false);
            var replacement = await WaitForSnapshotAsync(backend).ConfigureAwait(false);
            Check(replacement.Sessions[0].Id != original.Sessions[0].Id, "Session ID was reused across workers.");
            Check(replacement.Revision > original.Revision && replacement.SourcePolicyRevision == 7, "Revision or policy was lost on recovery.");
            Check(replacement.Sessions[0].MediaProperties.Title == "Initial", "The uncertain command was replayed.");
            Check((await backend.ExecuteAsync(Command(original, MediaOperation.Play), CancellationToken.None).ConfigureAwait(false)).Status == MediaBackendCommandStatus.SessionGone, "Old command reached the replacement.");
            Check(await backend.GetArtworkAsync(original.Sessions[0].MediaProperties.Artwork!.Value, CancellationToken.None).ConfigureAwait(false) is null, "Old artwork reached the replacement.");
    ...
    private static async Task CancellationAsync()
    {
        ...
        await EventuallyAsync(async () => (await backend.ReadSnapshotAsync(CancellationToken.None).ConfigureAwait(false)).Sessions[0].PlaybackState == MediaPlaybackState.Stopped).ConfigureAwait(false);
        await cancel.CancelAsync().ConfigureAwait(false);
        try { await command.ConfigureAwait(false); throw new InvalidOperationException("Cancellation was ignored."); }
        catch (OperationCanceledException) { }
        Check((await backend.ExecuteAsync(Command(snapshot, MediaOperation.Play), CancellationToken.None).ConfigureAwait(false)).Status == MediaBackendCommandStatus.Completed, "Cancellation blocked other commands.");
        Check(backend.WorkerEpoch == epoch, "Cancellation unnecessarily replaced the worker.");
    }

    private static async Task TimeoutAsync()
    {
        await using var backend = new OutOfProcessMediaBackend(Program.Options("hang-command") with { RequestTimeout = TimeSpan.FromMilliseconds(400) });
        await backend.StartAsync(CancellationToken.None).ConfigureAwait(false);
        var snapshot = await WaitForSnapshotAsync(backend).ConfigureAwait(false);
        var epoch = backend.WorkerEpoch;
        var result = await backend.ExecuteAsync(Command(snapshot, MediaOperation.SkipNext), CancellationToken.None).ConfigureAwait(false);
    ...
    private static async Task DisposalAsync()
    {
        ...
        Check(worker.HasExited && backend.ForcedTerminationCount == 0 && backend.RestartCount == 0, "Normal disposal failed or restarted a worker.");
    }
    ...
    private static async Task OwnerDeathAsync(string behavior, string directory)
    {
        ...
        {
            var workerId = 0;
            await EventuallyAsync(async () => File.Exists(report) && int.TryParse(await File.ReadAllTextAsync(report).ConfigureAwait(false), out workerId)).ConfigureAwait(false);
    ...
    private static async Task InvalidMessageAsync(bool oversized)
    {
        ...
        using var worker = OwnedWorkerProcess.Start(Environment.ProcessPath!, ["--worker", pipeName, owner.ToString("D"),
            Environment.ProcessId.ToString(System.Globalization.CultureInfo.InvariantCulture), "synthetic"]);
        using var deadline = new CancellationTokenSource(TimeSpan.FromSeconds(5));
        await pipe.WaitForConnectionAsync(deadline.Token).ConfigureAwait(false);
        var payload = Encoding.UTF8.GetBytes($$$"""{"kind":0,"version":99,"owner":"{{{owner:D}}}","backendId":"synthetic","policy":{"revision":0,"excludedApplicationIds":[]}}""");
        var header = new byte[4];
        BinaryPrimitives.WriteInt32LittleEndian(header, oversized ? int.MaxValue : payload.Length);
        await pipe.WriteAsync(header, deadline.Token).ConfigureAwait(false);
        if (!oversized) { await pipe.WriteAsync(payload, deadline.Token).ConfigureAwait(false); }
